...
class Matrix {
	...
	getInverse() {
		if (this.m == this.n && this.m == 2) return new Matrix(2,2,[this.entries[3], -this.entries[1], -this.entries[2], this.entries[0]].map(a => a / this.determinant))
...
function compileMatrices() {
	...
	for (const elem of elems) {
		const string = elem.ariaLabel || elem.attributes['aria-label'].value
		if (!string) continue
